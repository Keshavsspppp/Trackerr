...
export default function DashboardClient({
  ...
  async function handleRestore(app: IApplication) {
    try {
      // Exclude read-only metadata fields generated by the DB
      const { _id, userId, lastUpdated, createdAt, ...rest } = app;
      ...
      if (res.ok) {
        showToast(`Restored internship at ${app.company} ✓`, 'success');
        handleRefresh();
      } else {
        showToast('Failed to restore internship', 'error');
      }
    } catch {
      showToast('Network error while restoring — please try again', 'error');
    }
  }

  async function triggerDelete(app: IApplication) {
    const id = app._id;
    ...
    if (isDemo) {
      ...
      // Fire delete request immediately to prevent silent failures on tab close
  ...
  const limitParam = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('limit') : null;
  const currentLimit = limitParam ? parseInt(limitParam, 10) : 100;

  function handleLoadMore() {
    const nextLimit = (isNaN(currentLimit) ? 100 : currentLimit) + 100;
    router.replace(`/dashboard?limit=${nextLimit}`, { scroll: false });
  }
  ...
  return (
    <div className="layout-root">
      ...
      <aside
        className={`layout-sidebar${!sidebarOpen ? ' sidebar-collapsed' : ''}`}
        aria-label="Main navigation"
      >
          <div style={{
            padding: sidebarOpen ? "16px 16px 14px" : "14px 0",
            ...
            {sidebarOpen && (
              <div style={{ display: "flex", alignItems: "center", gap: "10px", minWidth: 0 }}>
                <img src="/logo.png" alt="Trackerr Logo" width="28" height="28" style={{ borderRadius: "50%", flexShrink: 0 }} />
                <span
                  style={{
                    fontSize: "20px",
                    fontWeight: 700,
            ...
            <button
              ...
              style={{
                ...
                borderRadius: "6px",
                transition: "background 150ms ease, transform 240ms ease",
                flexShrink: 0,
        ...
        >
          ...
          >
            <LayoutDashboard size={18} aria-hidden="true" style={{ flexShrink: 0 }} />
        ...
        <div
          style={{
            padding: sidebarOpen ? "16px 12px" : "12px 4px",
        ...
        >
          {sidebarOpen ? (
            /* Expanded: avatar + name + theme toggle + sign out */
            <>
              <div
                style={{
                  display: "flex",
      ...
      {/* ── Main content area ── */}
      <div
        className={`layout-main${!sidebarOpen ? ' sidebar-collapsed' : ''}`}
      >
        {/* Mobile top header */}
        <header
          style={{
          ...
          {view === "settings" ? (
            ...
            <>
              ...
              <section aria-labelledby="apps-heading">
                ...
                {visibleApps.length < stats.total && (
                  <div style={{ display: 'flex', justifyContent: 'center', marginTop: '24px' }}>
                    <button
                      onClick={handleLoadMore}
                      className="btn-secondary hover-btn-neutral"
                      style={{
                        padding: '10px 24px',
                        fontSize: '14px',
                        fontWeight: 600,
                    ...
                    >
                      Load More Internships ({visibleApps.length} of {stats.total} shown)
                    </button>
                  </div>
                )}
              </section>
